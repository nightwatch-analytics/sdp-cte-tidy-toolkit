...
cte_credit_data <- read_csv("data/cte_sample_credits.csv")
...
cte_credit_data$entry_pathway <- factor(cte_credit_data$entry_pathway,
  levels = c(1, 2, 3, 4),
...
avg_cum_attempt_earn <- cte_credit_data %>%
  ...
  summarise(
    cumcollegecreditsearned = mean(cumcollegecreditsearned, na.rm = TRUE),
...
ggplot(avg_cum_attempt_earn, aes(x = cohorttermindex)) +
  geom_line(aes(y = cumcollegecreditsearned),
    color = colors[1]
  ) +
  geom_line(aes(y = cumcollegecreditsattempted),
    color = colors[2], linetype = "dashed"
  ) +
  geom_line(aes(y = cumidealcreditsearned),
    color = colors[3]
  ) +
...
# For each student, calculate percentile of credits earned in the first term
...
first_term_cred <- first_term_cred %>%
  select(studentid, bin) %>%
  mutate(
    bin = ifelse(bin == 2 | bin == 3, 2, bin),
...
ggplot(avg_credit_bin, aes(x = cohorttermindex, y = cumcollegecreditsearned, color = factor(bin))) +
  ...
  labs(
    title = "Average College-Level Credit Accumulation by Entry Term College-Level Credits Earned",
    x = "Cohort Term",
  ...
  scale_x_continuous(limits = c(1, 6), breaks = seq(1, 6, 1)) +
  scale_color_manual(
    name = "Entry Term College-Level Credits Earned Percentile",
    values = colors,
